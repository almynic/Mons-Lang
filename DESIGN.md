...
**Beyond Phase 1 (still evolving):** the parser and type checker accept **inherent `impl`** and **trait `impl Trait for Type`** (MVP, non-generic). **Bytecode** compiles **structs**, **method calls** (including static trait dispatch via **`resolved_fn`**), richer **`use`** forms (plain/selective/glob), **`try`/`catch`/`finally`/`throw`** (with documented limits), and a **`stdlib/core.mons`** slice loaded like any other module. Still largely future work: **generics**, AST **macros**, a full **standard library**, and bytecode gaps for some **`match`** pattern forms — see README.
...
| 4 | **`try` / `catch` / `finally`** | 3 optional (orthogonal) | **Done (MVP on both backends)** — parse, typecheck, tree-walk eval, and bytecode VM support `try/catch/finally/throw`; bytecode lowering now supports `return` in try regions via unwind signaling (`smoke_try_return` in `--vm-test`), and `finally` runs on success and throw paths. |
| 5 | **Trait `impl Trait for Type` + dispatch** | 3 recommended (stable calls) | **Done (MVP)** — parse `trait { fn …; }`, `impl Trait for Type { … }`; checker registers traits, checks impl signatures vs trait (structural AST match on params/returns, including **`Self`** on the trait side vs the impl struct name), rejects generic traits/supertraits; **no vtable**: `r.m()` resolves by **static receiver type** to a single `NODE_FN_DECL` stored as **`method_call.resolved_fn`** (same index model as inherent `impl`); interpreter + bytecode use it; `tests/trait_impl.mons` + `smoke_trait_bump` in `--vm-test`; default trait methods / trait objects remain Phase 2+. |
...
- **Full generic** parity on **bytecode** (bounded type-parameter **method** calls may still require **`resolved_fn`** or eval-only paths), **trait objects**, default trait methods. *(Checker MVP: turbofish / `Ty[U]`, trait bounds on `T`, **`Self`** in trait types, bounded **`T`** field access — `tests/generics_trait_bounds.mons`.)*
...
# Phase 2 (closed scope): bytecode 2A–2C, reflection, stdlib/core.mons via use, hybrid GC, trait impl + match subset + try/catch/finally/throw (including return lowering in try regions) on both backends — see “Phase 2 closed scope (complete)” above
# Phase 2+: macro pass, full stdlib, register VM, remaining match/forms
```
